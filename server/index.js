...
async function fetchYahooQuote(item) {
  const response = await fetch(
    `https://query1.finance.yahoo.com/v7/finance/quote?symbols=${encodeURIComponent(item.symbol)}`,
  );
  ...
  const previousClose = quote?.regularMarketPreviousClose ?? quote?.regularMarketPreviousClose;
